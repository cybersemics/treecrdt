...
- `known_state` (delete metadata): writer-side subtree awareness snapshot used by defensive deletion. `delete` ops MUST
  ...
  `known_state` is computed at creation time, instead of hand-crafting raw delete ops. Its bytes MUST use the canonical
  [TreeCRDT VersionVector v0 encoding](../version-vector-v0.md).
