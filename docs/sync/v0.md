...
- `known_state` (delete metadata): writer-side subtree awareness snapshot used by defensive deletion. `delete` ops MUST
  ...
  `known_state` is computed at creation time, instead of hand-crafting raw delete ops. The field MUST contain canonical
  [TreeCRDT VersionVector v0 bytes](../version-vector-v0.md) directly. A canonical empty vector is valid; a zero-length
  byte field is not.
