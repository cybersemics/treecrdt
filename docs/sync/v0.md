...
- **Live updates semantics**: the repo currently implements “subscriptions” as periodic catch-up (`syncOnce` polling). Push-based subscriptions (`Subscribe`/`Unsubscribe`) are reserved for a later revision.
...
- **Live sync ("subscriptions")**: implemented as periodic `syncOnce()` polling, not as dedicated `Subscribe` wire messages.
...
- v0 in this repo implements live updates as periodic catch-up: repeatedly run the same `syncOnce(filter)` on an interval.
- This yields eventual convergence for both full-tree and partial-tree filters without requiring new wire messages.
- Push-based subscriptions (`Subscribe`/`Unsubscribe`) are reserved for a later revision; the `.proto` schema contains drafts, but the current `SyncPeer` implementation does not emit/handle them.
...
### Live Updates After Sync (Polling)

After reconciliation, a client can keep a view “live” by periodically running the same catch-up again with the same
filter.

Implementation note: in this repo, `SyncPeer.subscribe(transport, filter, { intervalMs })` is a convenience wrapper
around repeated `syncOnce()`. There are no dedicated on-wire `Subscribe`/`Unsubscribe` messages in the current
implementation.
