...
- The TypeScript **protocol reference** (peer, types, codecs, in-memory test harness) lives in `packages/sync-protocol/protocol` and is published as `@justthrowaway/sync-protocol`.
- **`@justthrowaway/sync`** (`packages/treecrdt-sync`) is a higher-level **client** package: it composes discovery (`@justthrowaway/discovery`), the protocol runtime, and the SQLite `SyncBackend` from `@justthrowaway/sync-sqlite` to connect over a spec `WebSocket` (global in browsers; in Node, pass e.g. `webSocketCtor` from `undici`; see package README). Use it when you want a single dependency for remote sync; use `@justthrowaway/sync-protocol` alone if you own transports and wiring.
