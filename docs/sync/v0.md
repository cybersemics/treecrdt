...
- `exchange_id` MUST be non-empty and unique among in-flight Hello exchanges on the transport.
...
- `exchange_id` MUST exactly match the `Hello.exchange_id` being acknowledged.
...
- `more` indicates the receiver consumed the latest batch but still needs more input:
  - `codewords_received`: number of codewords consumed so far
  - `credits`: how many additional `RibltCodewords` messages the initiator MAY send immediately
...
- `exchange_id` identifies a failed Hello when no `HelloAck` can be sent.
...
3. The receiver feeds codewords into the decoder until either:
   - it needs more input and sends `RibltStatus(more)` with additional sender credits,
   - it decodes the symmetric difference and sends `RibltStatus(decoded)`, or
   - it fails and sends `RibltStatus(failed)`.

While reconciliation is still in progress:

- The initiator SHOULD treat `RibltStatus(more)` as explicit backpressure / flow control and only send the number of
  additional `RibltCodewords` messages granted by `more.credits`.
- The receiver SHOULD send `RibltStatus(more)` whenever it has consumed a batch but cannot yet decode, instead of
  relying on transport latency or sender-side timers.
...
- The TypeScript **protocol reference** (peer, types, codecs, in-memory test harness) lives in `packages/sync-protocol/protocol` and is published as `@treecrdt/sync-protocol`.
- **`@treecrdt/sync`** (`packages/treecrdt-sync`) is a higher-level **client** package: it composes discovery (`@treecrdt/discovery`), the protocol runtime, and the SQLite `SyncBackend` from `@treecrdt/sync-sqlite` to connect over a spec `WebSocket` (global in browsers; in Node, pass e.g. `webSocketCtor` from `undici`; see package README). Use it when you want a single dependency for remote sync; use `@treecrdt/sync-protocol` alone if you own transports and wiring.
