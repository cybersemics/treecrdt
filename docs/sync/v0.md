...
- **IBLT interop details**: we need a precise, testable definition of how items are encoded, hashed, streamed, and decoded so independent implementations converge (including how `opRef` is derived).
...
- `opRef`: fixed-width identifier used inside IBLTs (16 bytes in v0; derivation from `opId` is TBD).
...
For v0 interop, define:

```
H(x) = blake3("treecrdt/ibltkey/v0" || x)[0..16]
```
...
IBLT reconciliation requires XOR-summing items, so items MUST be fixed-width bytes.
Because `replicaId` is variable-sized, `opId = (replicaId, counter)` is not a fixed-width byte string.

This protocol therefore uses `opRef` (`OpRef.bytes`, 16 bytes) as the IBLT item, and `IbltStatus.decoded` reports
missing sets as `opRef`s. Responders must be able to map `opRef → opId → Operation` to send `OpsBatch`.

**Open issue**: finalize the canonical derivation of `opRef` from `opId` for v0 interoperability. Candidate approaches:
- Make `replicaId` fixed-size so `opId` is fixed-width and can be used directly as the IBLT item.
- Standardize a domain-separated hash-based `opRef` over a canonical `(doc_id, replica_id, counter)` byte encoding.
...
### OpId / OpRef Encoding for IBLT

`opRef` derivation is intentionally left as an open issue for now (see `opRef` section above). Before independent
implementations can interoperate, this MUST be standardized so both peers compute identical `opRef`s for the same `opId`.
