...
- **`@treecrdt/sync`** (`packages/sync`) is a higher-level **client** package for browser-style apps: it composes discovery (`@treecrdt/discovery`), the protocol runtime, and the SQLite `SyncBackend` from `@treecrdt/sync-sqlite` to connect over WebSocket. Use it when you want a single dependency for remote sync; use `@treecrdt/sync-protocol` alone if you own transports and wiring.
