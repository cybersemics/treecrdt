...
- **Per-doc key bundles** sealed under the device wrap key:
  - a per-doc **issuer key** (Ed25519) used to issue capability tokens for that `doc_id`
  - a per-doc **replica key** (Ed25519) used to sign Sync v0 operations for that `doc_id`
  - (optional) a per-doc **payload key** (random 32 bytes) for E2EE payload encryption
