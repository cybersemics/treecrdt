...
All integers are unsigned and big-endian. `||` means concatenation, `bytes[n]` means exactly `n`
bytes, and `value[n]` means exactly `n` repetitions.

```text
vector = u8(0) || u32(n) || entry[n]
...
- The canonical empty vector contains no entries and is five bytes long. A zero-length byte string is
