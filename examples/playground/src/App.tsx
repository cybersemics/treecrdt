...
export default function App() {
  ...
  const onRemoteOpsApplied = React.useCallback(
    async (appliedOps: Operation[], affectedNodeIds: string[]) => {
      const benchAffectedNodes =
        affectedNodeIds.length > 0 ? affectedNodeIds : nodesAffectedByPayloadOps(appliedOps);
      ...
      recordBenchNodeTiming(benchAffectedNodes, { remoteOpsAppliedStartedAtMs: Date.now() });
      const active = clientRef.current ?? client;
      if (active && appliedOps.length > 0) {
        await hydratePayloadsForOps(active, appliedOps);
        recordBenchNodeTiming(benchAffectedNodes, { payloadsRefreshedAtMs: Date.now() });
      }
      ingestOps(appliedOps);
      recordBenchNodeTiming(benchAffectedNodes, { remoteOpsAppliedFinishedAtMs: Date.now() });
      ...
      if (canApplyAppendedChildren) {
        ...
        recordBenchNodeTiming(benchAffectedNodes, { treeRefreshAppliedAtMs: Date.now() });
      } else {
