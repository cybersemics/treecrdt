...
const REMOTE_PLACEHOLDER = "wss://sync.emhub.net/sync or ws://localhost:8787/sync";
...
test("public sync shortcut fills the shared dev sync endpoint", async ({ page }) => {
  ...
  await expect(remoteInput).toHaveValue("wss://sync.emhub.net/sync");
  await expect
    .poll(async () => {
      const url = new URL(page.url());
      return {
    ...
    .toEqual({ sync: "wss://sync.emhub.net/sync", transport: "hybrid" });
});
