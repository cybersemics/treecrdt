...
- `packages/treecrdt-sync`: high-level **client** sync for WebSocket + discovery + SQLite `SyncBackend` (npm: `@justthrowaway/sync`). Builds on `@justthrowaway/sync-protocol` and `@justthrowaway/discovery`.
- `packages/sync-protocol/protocol`: sync protocol/runtime core, transport-agnostic (npm: `@justthrowaway/sync-protocol`)
