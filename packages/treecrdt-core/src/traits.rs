...
pub trait NodeStore {
    ...
    ///
    /// Payload history is stored separately by [`PayloadStore`]. Tree-level defensive deletion
    /// combines this structural history with the current LWW payload writer.
    ...
    fn subtree_version_vector(&self, node: NodeId) -> Result<VersionVector> {
        ...
        while let Some(current) = pending.pop() {
            // A valid tree reaches every node once. Tracking visited nodes also makes this
            // derived-state helper safe when recovering from a malformed cycle or duplicate edge.
