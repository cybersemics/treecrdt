...
#[test]
fn subtree_version_vector_terminates_on_cycle_and_merges_each_node() {
    ...
    let subtree = nodes.subtree_version_vector(node_a).unwrap();
...
#[test]
fn subtree_version_vector_handles_a_deep_chain_without_recursion() {
    ...
    let subtree = nodes.subtree_version_vector(NodeId::ROOT).unwrap();
