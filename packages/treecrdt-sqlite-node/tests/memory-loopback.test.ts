...
test('syncs memory with SQLite, preserves authored operations, and reconciles incoming moves', async () => {
  const db = new Database(':memory:');
  loadTreecrdtExtension(db);
  const persistent = await createTreecrdtClient(db, { docId: 'loopback' });
