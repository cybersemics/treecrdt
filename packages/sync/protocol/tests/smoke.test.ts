...
test("subscribe pushes exact children-filter deltas for visible subtree updates without rescanning full state", async () => {
  ...
  try {
    const sub = pa.subscribe(ta, { children: { parent: rootBytes } }, { maxCodewords: 10_000, codewordsPerMessage: 256 });
    try {
      await waitUntil(() => a.hasOp(replicaHex.b, 1), { message: "expected initial subtree subscribe catch-up" });
      const scanCountAfterCatchUp = b.listOpRefsCalls;
...
test("subscribe pushes move-into-subtree deltas without rescanning when payload history is known", async () => {
  ...
  try {
    const sub = pa.subscribe(ta, { children: { parent: parentBBytes } }, { maxCodewords: 10_000, codewordsPerMessage: 256 });
    try {
      await sub.ready;
