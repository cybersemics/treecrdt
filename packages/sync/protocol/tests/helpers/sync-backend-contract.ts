...
import { deriveOpRefV0, type SyncBackend } from "@treecrdt/sync";
...
export function defineSyncBackendContract(
  ...
  test(`${label}: getOpsByOpRefs preserves request order and errors on missing`, async () => {
    ...
    try {
      ...
      await expect(backend.getOpsByOpRefs([new Uint8Array(16)])).rejects.toThrow(/missing|not found/i);
    } finally {
      await harness.close?.();
    }
  });
  ...
  test(`${label}: children(parent) includes the scope root payload opRef exactly once`, async () => {
    ...
    try {
      ...
      const payloadWriterHex = bytesToHex(
        deriveOpRefV0(docId, { replica, counter: 2n })
      );
