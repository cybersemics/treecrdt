...
function isChildrenFilter(filter: Filter): filter is Extract<Filter, { children: { parent: Uint8Array } }> {
...
export class SyncPeer<Op> {
  ...
  private recordSentEntries(sub: ResponderSubscription<Op>, entries: readonly PendingPushOp<Op>[]): void {
  ...
  private async pushSubscription(
    ...
    if (!opts.forceFullScan && isChildrenFilter(sub.filter) && opts.deltaOps && opts.deltaOps.length > 0) {
    ...
    for (let start = 0; start < newOpRefs.length; start += this.maxOpsPerBatch) {
      ...
      let sentEntries = chunk.map((opRef, i) => ({ opRef, opRefHex: bytesToHex(opRef), op: ops[i]! }));
      ...
      if (this.auth?.filterOutgoingOps && ops.length > 0) {
        ...
        ops = allowedOps;
        sentEntries = allowedRefs.map((opRef, i) => ({ opRef, opRefHex: bytesToHex(opRef), op: allowedOps[i]! }));
        chunk.length = 0;
  ...
  private async onSubscribe(
    ...
    try {
      ...
      if (isChildrenFilter(msg.filter) && opRefs.length > 0) {
        ...
        childrenState = { parentHex: bytesToHex(msg.filter.children.parent), visibleNodeHexes: new Set() };
      }
