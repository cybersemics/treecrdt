  ...
  return raw
    ...
    .map((entry) => {
      try {
        ...
        throw new Error(`${name} has invalid base64url key "${entry}": ${err instanceof Error ? err.message : String(err)}`);
      }
    });
}
...
async function main() {
  ...
  const authCapabilityIssuerPublicKeys = parseIssuerPublicKeysEnv("TREECRDT_SYNC_CWT_ISSUER_PUBKEYS");
  ...
  const pgNotifyChannel = process.env.TREECRDT_PG_NOTIFY_CHANNEL?.trim() || "treecrdt_sync_doc_updates";
