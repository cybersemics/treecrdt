...
declare const ownerGrantBrand: unique symbol;
/** Opaque canonical COSE bytes. Possession alone does not establish authorization. */
export type OwnerGrant = Uint8Array & { readonly [ownerGrantBrand]: true };

export type DirectOwnerGrantClaims = {
  docId: string;
...
/** @internal */
export function encodeOwnerGrantPayload(claims: DirectOwnerGrantClaims): Uint8Array {
...
/** @internal */
export function ownerGrantSignatureInput(payload: Uint8Array): Uint8Array {
  return encode(['Signature1', PROTECTED_HEADER, GRANT_DOMAIN, payload], rfc8949EncodeOptions);
}

/** @internal */
export function encodeOwnerGrant(payload: Uint8Array, signature: Uint8Array): OwnerGrant {
  return encode(
    [PROTECTED_HEADER, new Map(), payload, signature],
    rfc8949EncodeOptions,
  ) as OwnerGrant;
}
...
export async function verifyDirectOwnerGrant(opts: {
  ...
  if (!equalBytes(grant, encodeOwnerGrant(payload, signature))) {
    throw new Error('Non-canonical owner grant');
  }
  const claims = decodeCbor(payload);
  ...
  const parsed: DirectOwnerGrantClaims = {
    docId: claims.get(3),
    authorityPublicKey,
    replicaPublicKey,
    expiresAt: claims.get(4),
  };
  // Reconstruct the exact profile to reject extra claims and alternate key/header forms.
  if (!equalBytes(payload, encodeOwnerGrantPayload(parsed))) {
    throw new Error('Unsupported owner grant claims');
  }
  if (parsed.docId !== docId || !equalBytes(replicaPublicKey, expectedReplica)) {
    throw new Error('Owner grant document or replica mismatch');
  }
  if (nowSec >= parsed.expiresAt) throw new Error('Owner grant expired');
  if (!(await verifyEd25519(signature, ownerGrantSignatureInput(payload), authorityPublicKey))) {
    throw new Error('Invalid owner grant signature');
  }
  return {
    ...parsed,
    ownerGrantId: blake3(concatBytes(GRANT_ID_DOMAIN, grant)).slice(0, 16),
  };
}
