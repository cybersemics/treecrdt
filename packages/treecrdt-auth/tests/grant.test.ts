...
test('document identity and direct grant match the shared wire vector', async () => {
  ...
  for (const bytes of [
    ...
    const verified = await verifyGrant({ ...verification, grant: bytes });
...
test('a valid signature only authorizes its exact document and replica before expiry', async () => {
  for (const change of [
    ...
    await expect(verifyGrant({ ...verification, grant, ...change })).rejects.toThrow();
  }
  vi.spyOn(Date, 'now').mockReturnValue(vector.expiresAt * 1000);
  await expect(verifyGrant({ ...verification, grant, nowSec: undefined })).rejects.toThrow(
    /expired/,
  );
});
...
test('rejects unsigned headers, wrong algorithms and invalid signatures', async () => {
  ...
  await expect(verifyGrant({ ...verification, grant: new Uint8Array(1025) })).rejects.toThrow(
    /exceeds/,
  );
});
...
test('verification snapshots the grant and expected identity across awaits', async () => {
  ...
  const pending = verifyGrant(opts);
  opts.grant.fill(0);
...
test('the memory provider creates separate opaque keys and issues a verifiable direct grant', async () => {
  ...
  const authorityKey = await provider.createAuthorityKey();
  ...
  const pending = provider.issueGrant({
    authorityKey,
    replicaPublicKey,
    expiresAt: vector.expiresAt,
  });
...
test('provider rejects forged, foreign, wrong-role and deleted key handles', async () => {
  ...
  const authorityKey = await provider.createAuthorityKey();
  const replicaKey = await provider.createReplicaKey(authorityKey.docId);
  const foreign = await createMemoryAuthKeyProvider().createAuthorityKey();
  for (const key of [{ ...authorityKey }, foreign, replicaKey]) {
    await expect(
      provider.issueGrant({
        authorityKey: key as AuthorityKey,
        replicaPublicKey,
        expiresAt: vector.expiresAt,
      }),
