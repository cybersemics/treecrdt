...
function parseWorkloadList(raw: string, allowed: Set<WorkloadName>): WorkloadName[] {
  ...
  if (invalid.length > 0) {
    throw new InvalidArgumentError(`invalid workload(s): ${invalid.join(", ")} (allowed: ${WORKLOAD_NAMES.join(", ")})`);
  }
...
export function parseBenchCliArgs(opts: {
  ...
  const program = new Command()
    ...
    .option(
      "--count <n>",
      "shorthand for --sizes <n>",
      (val) => {
        const n = Number(val);
        if (!Number.isFinite(n) || n <= 0) throw new InvalidArgumentError(`invalid --count value: ${val}`);
        return n;
      }
    )
    .option(
    ...
    .option(
      "--workload <name>",
      `single workload (${WORKLOAD_NAMES.join(", ")})`,
      (val) => {
        if (!allowed.has(val as WorkloadName)) {
          throw new InvalidArgumentError(`invalid --workload value: ${val} (allowed: ${WORKLOAD_NAMES.join(", ")})`);
        }
