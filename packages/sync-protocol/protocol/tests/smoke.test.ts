...
test('syncOnce schedules remote cleanup when Hello send cancels synchronously', async () => {
  ...
  await expect(
    peer.syncOnce(control.transport, { all: {} }, { signal: controller.signal }),
  ).rejects.toThrow('cancelled inside Hello send');
  await waitUntil(() => control.sent.some((message) => message.payload.case === 'error'));

  expect(control.sent.map((message) => message.payload.case)).toEqual(['hello', 'error']);
});
