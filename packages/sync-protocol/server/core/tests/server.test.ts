...
import { SyncPeer, type SyncBackend, type SyncMessage } from '@justthrowaway/sync-protocol';
import type { WireCodec } from '@justthrowaway/sync-protocol/transport';
...
test('status endpoint returns provided status payload', async () => {
  const server = await startWebSocketSyncServer({
    ...
    statusInfo: async () => ({
      ok: true,
      service: '@justthrowaway/sync-server-postgres-node',
      version: '0.0.1',
      gitSha: 'abc123',
    }),
  });
