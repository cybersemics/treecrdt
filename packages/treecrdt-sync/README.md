...
`connectTreecrdtWebSocketSync` needs a spec `WebSocket` client: the global in browsers, or **`webSocketCtor`** (e.g. `import { WebSocket } from "undici"`) in Node. For a fully custom path, use **`createTreecrdtWebSocketSyncFromTransport`**, which only needs a `DuplexTransport` (no `WebSocket`). Custom transports must provide an idempotent `close` method and a read-only `closeSignal`. The high-level sync handle takes ownership, closes the transport when the handle closes, and does not permit reusing that transport object for another connection.
