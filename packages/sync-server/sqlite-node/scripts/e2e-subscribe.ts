...
async function connectPeer(docId: string, wsUrl: string, backend: SyncBackend<Operation>) {
  ...
  return { ws, transport, peer, close };
}
...
async function main() {
  ...
  const opA1: Operation = {
    meta: { id: { replica: "clientA", counter: 1 }, lamport: 1 },
    kind: { type: "insert", parent: ROOT_NODE_ID_HEX, node: nodeA1, position: 0 },
  };
  ...
  const rootInsertOnB = await withTimeout(gotRootInsertOnB.promise, 2_000, "B received root insert via push");

  nodeA1Child = randomNodeId();
  const opB1: Operation = {
    meta: { id: { replica: "clientB", counter: 1 }, lamport: 1 },
    kind: { type: "insert", parent: nodeA1, node: nodeA1Child, position: 0 },
  };
  await backendB.applyOps([opB1]);
  await b.peer.syncOnce(b.transport, childrenFilter(nodeA1));

  const childInsertOnA = await withTimeout(gotChildInsertOnA.promise, 2_000, "A received child insert via push");
  ...
  assert.equal(rootInsertOnB.kind.type, "insert");
  assert.equal(rootInsertOnB.kind.parent, ROOT_NODE_ID_HEX);
  assert.equal(rootInsertOnB.kind.node, nodeA1);
  assert.equal(childInsertOnA.kind.type, "insert");
  assert.equal(childInsertOnA.kind.parent, nodeA1);
  assert.equal(childInsertOnA.kind.node, nodeA1Child);
}
