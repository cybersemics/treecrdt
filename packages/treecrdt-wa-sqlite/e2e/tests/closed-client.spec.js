...
test('closed client rejects further calls and close/drop are idempotent', async ({ page, }, testInfo) => {
    ...
    await page.goto('/');
    await page.waitForFunction(() => typeof window.__runClosedClientE2E === 'function');
    const result = await page.evaluate(async () => {
        const runner = window.__runClosedClientE2E;
        if (!runner)
            throw new Error('__runClosedClientE2E not available');
        return await runner();
    });
