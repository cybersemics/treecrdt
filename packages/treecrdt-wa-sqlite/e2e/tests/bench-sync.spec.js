...
import { DEFAULT_SYNC_BENCH_ROOT_CHILDREN_WORKLOADS, DEFAULT_SYNC_BENCH_WORKLOADS, syncBenchRootChildrenSizesFromEnv, syncBenchSizesFromEnv, } from '@treecrdt/benchmark';
...
test('wa-sqlite sync OPFS benchmarks', async ({ page }) => {
    ...
    const results = await page.evaluate(async ({ rootChildrenSizes, sizes, baseWorkloads, rootChildrenWorkloads }) => {
        ...
        if (!runner)
            throw new Error('runTreecrdtSyncBench not available');
