...
// The sync bench runs on in-memory clients only: sync peers must share a docId,
// and a docId now maps to a single OPFS store.
test('wa-sqlite sync memory (browser) benchmarks', async ({ page }) => {
  ...
  const results = await page.evaluate(
    async ({ rootChildrenSizes, sizes, baseWorkloads, rootChildrenWorkloads }) => {
      ...
      const base = await runner(sizes, baseWorkloads);
      const rootChildren = await runner(rootChildrenSizes, rootChildrenWorkloads);
