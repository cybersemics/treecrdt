...
test('wa-sqlite OPFS single-owner WAL benchmarks', async ({ page }) => {
  ...
  const results = await page.evaluate(async () => {
    const runner = window.runWaSqliteBench;
    if (!runner) throw new Error('runWaSqliteBench not available');
    return await runner('browser-opfs-single-owner-wal');
  });

  expect(Array.isArray(results)).toBeTruthy();
  const repoRoot = repoRootFromImportMeta(import.meta.url, 4);
  const outDir = path.join(repoRoot, 'benchmarks', 'wa-sqlite-opfs-single-owner-wal');
...
test('wa-sqlite OPFS write-ahead VFS benchmarks', async ({ page }) => {
  test.setTimeout(180_000);
  page.on('console', (msg) => console.log(`[page][${msg.type()}] ${msg.text()}`));
  await page.goto('/');
  await page.waitForFunction(() => typeof window.runWaSqliteBench === 'function');
  ...
  const results = await page.evaluate(async () => {
    const runner = window.runWaSqliteBench;
    if (!runner) throw new Error('runWaSqliteBench not available');
    return await runner('browser-opfs-write-ahead');
  });

  expect(Array.isArray(results)).toBeTruthy();
  const repoRoot = repoRootFromImportMeta(import.meta.url, 4);
  const outDir = path.join(repoRoot, 'benchmarks', 'wa-sqlite-opfs-write-ahead');
