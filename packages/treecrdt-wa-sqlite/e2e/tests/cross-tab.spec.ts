...
async function openClient(page: Page, docId: string) {
  return page.evaluate(async (openDocId) => {
    ...
    return await open({ docId: openDocId });
  }, docId);
}
...
test('shared OPFS clients propagate materialization events across tabs', async ({
  context,
}, testInfo) => {
  ...
  const docId = `e2e-cross-tab-${suffix}`;
  ...
  try {
    ...
    const [summaryA, summaryB] = await Promise.all([
      openClient(pageA, docId),
      openClient(pageB, docId),
    ]);
