...
test.describe.serial('non-root base path', () => {
    ...
    test('memory client loads from base path (dev)', async ({ page }, testInfo) => {
        ...
        const summary = await page.evaluate(async () => {
            const fn = window.__createTreecrdtClient;
            if (!fn)
                return null;
    ...
    test('opfs client uses worker mode in preview build with base path', async ({ page, }, testInfo) => {
        if (testInfo.project.name !== 'chromium-basepath-preview')
            test.skip();
        await page.goto('/base-path/');
        await page.waitForSelector('[data-testid="run-demo"]', { timeout: 30000 });
        const summary = await page.evaluate(async () => {
            const fn = window.__createTreecrdtClient;
            if (!fn)
                return null;
    ...
    test("opfs init fails when OPFS VFS chunk can't load (and throws)", async ({ page, }, testInfo) => {
        if (testInfo.project.name !== 'chromium-basepath-preview')
            test.skip();
        ...
        const result = await page.evaluate(async () => {
            const fn = window.__createTreecrdtClient;
            if (!fn)
                return { ok: false, message: '__createTreecrdtClient missing' };
            try {
                await fn('opfs');
