...
import { buildWorkloads, runWorkloads } from '@justthrowaway/benchmark';
import { parseBenchCliArgs, repoRootFromImportMeta, writeResult } from '@justthrowaway/benchmark/node';
...
async function main() {
  ...
  for (const result of results) {
    const outFile =
      opts.outFile ?? path.join(repoRoot, 'benchmarks', 'wa-sqlite', `memory-${result.name}.json`);
