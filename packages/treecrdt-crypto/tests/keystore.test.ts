...
test('keystore v1: open issuer key fails with wrong docId (AAD mismatch)', async () => {
  ...
  const sealed = await sealTreecrdtIssuerKeyV1({
    wrapKey,
    docId: 'doc-a',
    issuerSk: issuer.issuerSk,
  });
...
test('keystore v1: seal/open local identity roundtrip', async () => {
  ...
  const wrapKey = generateTreecrdtDeviceWrapKeyV1();
  const identity = await generateTreecrdtLocalIdentityV1({
    docId,
    replicaLabel,
    localTokens: [new Uint8Array([1, 2, 3])],
  });
...
test('keystore v1: open local identity fails with wrong replicaLabel (AAD mismatch)', async () => {
  ...
  await expect(
    openTreecrdtLocalIdentityV1({ wrapKey, docId: 'doc-a', replicaLabel: 'replica-b', sealed }),
  ).rejects.toThrow();
});
