...
export async function generateTreecrdtDocKeyBundleV1(opts: {
  docId: string;
...
async function decodeDocKeyBundleV1(bytes: Uint8Array): Promise<TreecrdtDocKeyBundleV1> {
  ...
  const issuerSk = assertLen(
    assertBytes(get(map, 'issuer_sk'), 'DocKeyBundleV1.issuer_sk'),
...
export async function sealTreecrdtDocKeyBundleV1(opts: {
  ...
  if (!opts.bundle.docId || opts.bundle.docId.trim().length === 0)
    throw new Error('bundle.docId must not be empty');
  ...
  const ciphertext = await aesGcmEncrypt({
    ...
    nonce,
    plaintext,
    aad: sealedAadV1(opts.bundle.docId),
  });
...
export async function openTreecrdtDocKeyBundleV1(opts: {
  ...
  const nonce = assertLen(
    assertBytes(get(map, 'nonce'), 'SealedDocKeyBundleV1.nonce'),
    AES_GCM_NONCE_LEN,
    'nonce',
  );
...
export async function sealTreecrdtDocPayloadKeyV1(opts: {
  ...
  const ciphertext = await aesGcmEncrypt({
    ...
    nonce,
    plaintext,
    aad: sealedDocPayloadAadV1(opts.docId),
  });
...
export async function openTreecrdtDocPayloadKeyV1(opts: {
  ...
  const nonce = assertLen(
    assertBytes(get(map, 'nonce'), 'SealedDocPayloadKeyV1.nonce'),
    AES_GCM_NONCE_LEN,
    'nonce',
  );
  ...
  const payloadKey = assertLen(
    assertBytes(get(claims, 'payload_key'), 'DocPayloadKeyV1.payload_key'),
    DOC_PAYLOAD_KEY_LEN,
    'payload_key',
  );
...
export async function generateTreecrdtIssuerKeyV1(opts: {
  docId: string;
...
async function decodeIssuerKeyV1(bytes: Uint8Array): Promise<TreecrdtIssuerKeyV1> {
  ...
  const issuerSk = assertLen(
    assertBytes(get(map, 'issuer_sk'), 'IssuerKeyV1.issuer_sk'),
    ED25519_SECRET_KEY_LEN,
    'issuer_sk',
  );
...
export async function sealTreecrdtIssuerKeyV1(opts: {
  ...
  const ciphertext = await aesGcmEncrypt({
    ...
    nonce,
    plaintext,
    aad: sealedIssuerAadV1(opts.docId),
  });
...
export async function openTreecrdtIssuerKeyV1(opts: {
  ...
  const nonce = assertLen(
    assertBytes(get(map, 'nonce'), 'SealedIssuerKeyV1.nonce'),
    AES_GCM_NONCE_LEN,
    'nonce',
  );
...
export async function generateTreecrdtLocalIdentityV1(opts: {
  ...
  ensureEd25519();
  if (!opts.docId || opts.docId.trim().length === 0) throw new Error('docId must not be empty');
  if (!opts.replicaLabel || opts.replicaLabel.trim().length === 0)
    throw new Error('replicaLabel must not be empty');
...
async function decodeLocalIdentityV1(bytes: Uint8Array): Promise<TreecrdtLocalIdentityV1> {
  ...
  const localSk = assertLen(
    assertBytes(get(map, 'local_sk'), 'LocalIdentityV1.local_sk'),
    ED25519_SECRET_KEY_LEN,
    'local_sk',
  );
  ...
  if (localTokensRaw !== undefined && localTokensRaw !== null) {
    if (!Array.isArray(localTokensRaw))
      throw new Error('LocalIdentityV1.local_tokens must be an array');
    localTokens = localTokensRaw.map((v, i) =>
      assertBytes(v, `LocalIdentityV1.local_tokens[${i}]`),
    );
  }
...
function sealedLocalIdentityAadV1(docId: string, replicaLabel: string): Uint8Array {
  return concatBytes(
    concatBytes(
      concatBytes(SEALED_LOCAL_IDENTITY_V1_AAD_DOMAIN, utf8ToBytes(docId)),
      SEALED_LOCAL_IDENTITY_V1_AAD_SEP,
    ),
...
export async function openTreecrdtLocalIdentityV1(opts: {
  ...
  const nonce = assertLen(
    assertBytes(get(map, 'nonce'), 'SealedLocalIdentityV1.nonce'),
    AES_GCM_NONCE_LEN,
    'nonce',
  );
  ...
  if (identity.replicaLabel !== opts.replicaLabel)
    throw new Error('LocalIdentityV1.replica_label mismatch');
  return identity;
}
